...
process = cms.Process("MULTITRACKVALIDATORFORCA")
...
#process.multiTrackValidator.outputFile = cms.string('valiplots.root')
...
process.cutsRecoTracks.algorithm = cms.vstring('iter0')
...
#CAtrackgeneration GEN-SIM-RECO
process.localreco = cms.Sequence(process.siPixelRecHits*process.siStripMatchedRecHits)
process.newtracks = cms.Sequence(process.MeasurementTrackerEvent*process.offlineBeamSpot*process.CAglobaltracks)
process.CAtrackgeneration = cms.Sequence(process.localreco*process.siPixelClusterShapeCache*process.standalonemuontracking*process.caloTowersRec*process.newtracks)


# paths
process.val = cms.Path(
	  process.CAtrackgeneration
