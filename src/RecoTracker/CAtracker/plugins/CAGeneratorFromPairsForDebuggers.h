#ifndef CAGeneratorFromPairs_h
#define CAGeneratorFromPairs_h
...
//
// CAGeneratorFromPairs class declaration
//


class CAGeneratorFromPairs : public MultiHitGenerator {
   public:
      using ConstRecHitPointer = BaseTrackerRecHit const *;
    
      CAGeneratorFromPairs(const edm::ParameterSet& ,  edm::ConsumesCollector& iC);
      ~CAGeneratorFromPairs();
	...
	void fitPairs(CAcell *pair, const GlobalPoint& vertex, edm::ESHandle<MagneticField> & mf);
...
int max_status; //Now set to 10, in general to build n-tuplets set to > n-2
    ...
    int tmp_prodCells43;
    int tmp_prodCells54;
    int tmp_prodCells1034;
    int tmp_prodCells_1026;
